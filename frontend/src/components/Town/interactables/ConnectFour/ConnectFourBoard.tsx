    ...
    <StyledConnectFourBoard aria-label='Connect Four Board'>
      {board.map((row, rowIndex) => {
        return row.map((cell, colIndex) => {
          let boardVal = board[rowIndex][colIndex] as unknown;
          if (boardVal !== 'Red' && boardVal !== 'Yellow') {
            boardVal = 'Empty';
          }
          ...
          return (
            <StyledConnectFourSquare
              ...
              disabled={!isOurTurn}
              aria-label={`Cell ${rowIndex},${colIndex} (${boardVal})`}
              style={{ backgroundColor: cell ? cell : 'ButtonFace' }}>
              {cell}
            </StyledConnectFourSquare>
          );
        });
